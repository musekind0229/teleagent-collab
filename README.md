...
- **Grok 真机 lead**：`bin/run-live-grok-lead.py`；Claude/Codex CLI 为 stub 待办；DeepSeek harness 包装接公网 `dsh` headless（无 bin/key 则 fail-closed；live 模型未在本仓默认验收）
...
src/lead_adapter/     # 组长协议：grok_cli / inprocess / deepseek_harness（条3）
...
普通权限与产物验收由 Grok 自动处理；Question、系统动作和组长失败会出现在 decision API。`--teleagent-stdin-wrap` 当前是诊断兼容通道：实机已创建真实 TeleAgent session，但 GUI 模型授权复用仍返回 `HTTP 401 / invalid token`，尚未完成真实产物闭环。重新登录后发送消息不是规定步骤，也不能修复该问题。
